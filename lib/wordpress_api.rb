module WordpressApi
    ...
    def get_courses(ids)
        response = HTTParty.get("#{ENV['WORDPRESS_DOMAIN']}/wp-json/wp/v2/intake?status=draft,publish,trash&per_page=100&include=#{ids.join(",")}", {
            basic_auth: @@auth
        })
    ...
    def get_vacancies(ids)
        response = HTTParty.get("#{ENV['WORDPRESS_DOMAIN']}/wp-json/wp/v2/vacancy?status=draft,publish,trash&per_page=100&include=#{ids.join(",")}", {
            basic_auth: @@auth
        })
        response.parsed_response
    end
